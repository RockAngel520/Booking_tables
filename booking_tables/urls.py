...
from booking_tables.views import (
    AboutListView,
    BookingCreateView,
    BookingDeleteView,
    BookingDetailView,
...
urlpatterns = [
    ...
    path(
        "booking_update/<int:pk>/", BookingUpdateView.as_view(), name="booking_update"
    ),
    path(
        "booking_confirm_delete/<int:pk>/",
        BookingDeleteView.as_view(),
        name="booking_confirm_delete",
    ),
    path("booking_create/", BookingCreateView.as_view(), name="booking_create"),
    path(
        "api/check-table-availability/",
        views.check_table_availability,
